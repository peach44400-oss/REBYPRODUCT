...
$("nav").addEventListener("click", e => {
  ...
  // 출고/생산 스케줄에서 저장 안 한 변경이 있으면, 다른 화면(또는 다른 종류)로 넘어가기 전에 확인
  const _schedSelf = SCHED.kind === "prod" ? "prodsched" : "sched";
  if (!_navBypass && curScr === "scr-sched" && b.dataset.scr !== _schedSelf && SCHED.dirty) {
  ...
  const fn = { dash: loadDash, prod: loadProd, ship: loadShip, entry: openEntry, lot: loadLot, items: renderMasters, ana: loadAna, lookup: () => lkCal.render(),
    postat: loadPoStat, memos: loadMemos, matstat: loadMatStatus, sched: () => { SCHED.kind = "ship"; _schedApplyKindUI(); loadSchedule(); }, prodsched: () => loadProdSched(), invoice: loadInvoice, recv: loadReceivables, salesorder: loadSalesOrders, pcard: loadPCard, pnl: loadPnl, workorder: loadWorkorder, salesrep: loadSalesRep, purchrep: loadPurchRep,
...
function navEnsureGroupOpen(scr) {
  const btn = document.querySelector(`#nav button[data-scr="${scr}"]`);
  if (!btn) return;
  const grp = btn.closest(".nav-grp");
  if (grp) { const open = navOpenSet(); if (!open.has(grp.dataset.grp)) { open.add(grp.dataset.grp); navSaveSet(open); applyNavGroups(); } }
  const sub = btn.closest(".nav-subgrp");   // 하위 그룹도 열어줌
  if (sub) { const so = navSubOpenSet(); if (!so.has(sub.dataset.subgrp)) { so.add(sub.dataset.subgrp); navSubSaveSet(so); applyNavSub(); } }
}
// 하위 그룹(nav-subsep/nav-subgrp) 접기·펼치기 — 상위 그룹과 별도 저장
const NAVSUB_KEY = "navOpenSub";
function navSubOpenSet() { try { return new Set(JSON.parse(localStorage.getItem(NAVSUB_KEY) || "[]")); } catch (e) { return new Set(); } }
function navSubSaveSet(set) { try { localStorage.setItem(NAVSUB_KEY, JSON.stringify([...set])); } catch (e) {} }
function applyNavSub() {
  const open = navSubOpenSet();
  document.querySelectorAll('#nav .nav-subsep[data-subgrp]').forEach(sep => {
    const key = sep.dataset.subgrp, on = open.has(key);
    const grp = document.querySelector(`#nav .nav-subgrp[data-subgrp="${key}"]`);
    sep.classList.toggle("collapsed", !on);
    if (grp) grp.classList.toggle("collapsed", !on);
  });
}
$("nav").addEventListener("click", e => {
  ...
  const sub = e.target.closest(".nav-subsep[data-subgrp]");
  if (sub) { const o = navSubOpenSet(); o.has(sub.dataset.subgrp) ? o.delete(sub.dataset.subgrp) : o.add(sub.dataset.subgrp); navSubSaveSet(o); applyNavSub(); return; }
...
$("matExpList").addEventListener("click", async e => {
  ...
  if (dz) {
    ...
    openDisposeModal({ mid: r.id, name: r.name, unit: r.unit || "", expired: r.onhand_expired,
      expiry: r.expiry || "", onReload: () => loadDash() });
    return;
  }
  ...
  h += top.map(r => {
    if (r.plan_qty > 0) {
      ...
      return `<div class="abar-row num">
        ...
        <span class="abar-pct" style="color:${color}">${pct.toFixed(1)}% <small>생산 ${NF(r.prod_qty)} / 계획 ${NF(r.plan_qty)} · ${gap >= 0 ? "+" + NF(gap) + " 초과" : NF(-gap) + " 미달"}</small></span></div>`;
    }
    return `<div class="abar-row num">
      <span class="abar-name" title="${esc(r.name)}">${esc(r.name)}</span>
...
let ENTRY_STALE = false;   // 발주 입고/입고 취소 등 다른 화면에서 일일 데이터가 바뀌면 true → 일일 입력 진입 시 자동 재로드
function openEntry() {
  ...
  if (!E.date) {
    ...
    // 다른 화면(발주 현황 입고/취소)에서 이 날짜 데이터가 바뀜 — 다시 클릭할 필요 없이 자동 갱신
    ENTRY_STALE = false;
    loadDay(E.date);
  }
  entryCal.render();
...
async function loadDay(date) {
  ...
  E.matExpiry = d.mat_expiry || {};      // {material_id: {onhand, expired, batches[{exp,qty}]}} — 유통기한 만료 재고
...
function matOptGroups(val) {
  ...
  const semis = (M.semi || []).filter(s => s.status !== "단종" && s.status !== "중단");
  return grp("원재료", M.raw) + grp("부재료", M.sub) + grp("🧫 반제품", semis);
}
...
/* ── 반제품 생산 (반제품=직접 만드는 자재) — 배합수 입력 → 반제품 재고↑ + 원재료 자동 차감(백엔드가 semi_bom대로).
   프론트는 미리보기만 하고, 실제 차감/증가는 저장 시 서버가 계산한다. ── */
let SEMIBOM = {};   // material_id → [{material_id, qty_per_unit, name, unit}] (반제품 레시피 캐시)
async function ensureSemiBom(mid) {
  if (!mid || SEMIBOM[mid]) return;
  try { SEMIBOM[mid] = ((await api("/api/semibom/" + mid)) || {}).rows || []; }
  catch (e) { SEMIBOM[mid] = []; }
}
function renderSemiProd() {
  const box = $("eSemiProd"); if (!box) return;
  const semis = (M.semi || []).filter(p => p.status !== "단종" && p.status !== "중단");
  $("qaSemiProdList").innerHTML = semis.map(o => `<option value="${esc(o.name)}">`).join("");
  const semiSel = sid => `<select class="mini-input nm" data-sf="material_id" style="min-width:150px">
  ...
  box.innerHTML = (E.semiProd || []).map((r, i) => {
    ...
    const by = Number(sm.batch_yield) || 0;
    const batches = Number(String(r.batches).replace(/,/g, "")) || 0;
    const made = batches * by;
    const recipe = SEMIBOM[r.material_id];
    const raws = (recipe || []).map(b => {
      const m = materialById(b.material_id) || {};
      let q = batches * (Number(b.qty_per_unit) || 0);
      const su = (b.unit || "g").toLowerCase(), mu = (m.unit || "").toLowerCase();   // 레시피 단위 → 자재 재고 단위
      if (su === "g" && mu === "kg") q /= 1000;
      else if (su === "kg" && mu === "g") q *= 1000;
      return `${esc(m.name || b.name || "?")} ${NF(q)}${esc(m.unit || su)}`;
    });
    const preview = !r.material_id ? "반제품을 선택하세요"
...
wireQuickAdd("qaSemiProd", "qaSemiProdList", () => (M.semi || []).filter(p => p.status !== "단종" && p.status !== "중단"), async hit => {
  ...
  E.semiProd.push({ material_id: hit.id, batches: "" });
  await ensureSemiBom(hit.id); renderSemiProd();
}, "addSemiProd", () => { E.semiProd.push({ material_id: null, batches: "" }); renderSemiProd(); });
$("eSemiProd").addEventListener("input", e => {   // 입력 중엔 값만 반영(포커스 유지) — 미리보기는 blur/선택 시 갱신
  const tr = e.target.closest("tr[data-si]"); if (!tr) return;
  const r = E.semiProd[+tr.dataset.si]; if (!r) return;
  if (e.target.dataset.sf === "batches") r.batches = e.target.value;
});
$("eSemiProd").addEventListener("change", async e => {
  const tr = e.target.closest("tr[data-si]"); if (!tr) return;
  const r = E.semiProd[+tr.dataset.si]; if (!r) return;
  if (e.target.dataset.sf === "material_id") {
    r.material_id = e.target.value ? +e.target.value : null;
    await ensureSemiBom(r.material_id);
  }
  renderSemiProd();   // 배합수 입력 확정(blur)·반제품 선택 후 생산량·원재료 미리보기 갱신
});
$("eSemiProd").addEventListener("click", e => {
  const del = e.target.closest("[data-sdel]"); if (!del) return;
  E.semiProd.splice(+del.closest("tr[data-si]").dataset.si, 1); renderSemiProd();
});
/* 생산 LOT 소비기한 분할 모달 */
...
// 유통기한 만료 재고 경고 + 확정 폐기 버튼 (자동/실사 행 공통) · 그날 폐기 기록·취소
function matExpiryHtml(mid) {
  if (!mid) return "";
  const unit = (materialById(mid) || {}).unit || "";
  const ex = (E.matExpiry || {})[mid];
  const disp = (E.matDisposal || []).filter(x => x.material_id === mid);
  const canStock = typeof MYDUTY !== "undefined" && MYDUTY.has && MYDUTY.has("stock");
  let html = "";
  if (ex && ex.expired > 0) {
    const exps = (ex.batches || []).map(b => b.exp).filter(Boolean);
    const el = exps.length ? ` · 기한 ${exps.join(", ")} 지남` : "";
    html += `<div style="margin-top:3px; font-size:11px; color:var(--crit); display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
      <span title="유통기한이 지난 재고입니다 — 확정 폐기하면 실재고에서 차감되어 더 이상 사용되지 않습니다">⚠ 만료 <b>${NF(ex.expired)}${esc(unit)}</b>${el}</span>
      ${canStock ? `<button type="button" class="btn ghost sm" data-matdispose="${mid}" style="color:var(--crit); border-color:var(--crit); padding:1px 8px;">확정 폐기</button>` : ""}</div>`;
  }
  if (disp.length) {
    const tot = disp.reduce((s, x) => s + (Number(x.qty) || 0), 0);
    html += `<div style="margin-top:2px; font-size:11px; color:var(--muted);">🗑 폐기 ${NF(tot)}${esc(unit)}${canStock ? disp.map(x =>
      `<button type="button" class="btn ghost sm" data-matdispundo="${x.id}" style="padding:0 6px; margin-left:4px;" title="${esc((x.reason || "") + " " + (x.expiry || ""))} — 클릭하면 취소(재고 복구)">취소</button>`).join("") : ""}</div>`;
  }
  return html;
}
function renderMat() {
  ...
  const rows = E.mat.map((r, i) => {
    ...
    return `<tr data-i="${i}">
      ...
      </div>${matExpiryHtml(r.material_id)}</td>
      <td class="auto">${esc(m.unit || "")}</td>
  ...
  const autoRows = (E.autoMat || []).filter(r => !manualIds.has(r.material_id)).map(r => `<tr style="color:var(--muted)">
    <td>${esc(r.name)} <span class="chip cat">자동 반영</span>${matExpiryHtml(r.material_id)}</td>
  ...
  // 이 날짜에 실사·자동 행이 없어도 유통기한 만료 재고(또는 폐기 기록)가 있는 자재는 별도 행으로 노출 → 확정 폐기 가능
  const shownIds = new Set([...E.mat.map(r => r.material_id), ...(E.autoMat || []).map(r => r.material_id)].filter(Boolean));
  const extraIds = [...new Set([...Object.keys(E.matExpiry || {}).map(Number),
                                ...(E.matDisposal || []).map(x => x.material_id)])].filter(mid => !shownIds.has(mid));
  const expRows = extraIds.map(mid => {
    const m = materialById(mid) || {};
    const oh = (E.matExpiry[mid] || {}).onhand;
    return `<tr style="color:var(--muted)"><td>${esc(m.name || ("#" + mid))}${matExpiryHtml(mid)}</td>
      <td>${esc(m.unit || "")}</td>
      <td class="r">${oh != null ? NF(oh) : "—"}</td>
      <td colspan="6" class="hint" style="white-space:normal">이 날짜엔 입력 기록이 없지만 유통기한 만료 재고가 있습니다 — 확정 폐기로 재고에서 차감하세요</td></tr>`;
  }).join("");
  $("eMat").innerHTML = (rows + autoRows + expRows)
...
$("eMat").addEventListener("click", e => {
  // 만료 폐기 버튼은 실사·자동 행 어디에 있든 여기서 처리 (tr[data-i] 가드보다 먼저)
  const dz = e.target.closest("[data-matdispose]");
  if (dz) { e.stopPropagation(); openMatDispose(+dz.dataset.matdispose); return; }
  const du = e.target.closest("[data-matdispundo]");
  if (du) { e.stopPropagation(); undoMatDispose(+du.dataset.matdispundo); return; }
...
// 유통기한 만료분 확정 폐기 — 폐기 수량·폐기일(기본=만료 시작일=유통기한+1)·사유를 골라 실재고에서 차감
...
function openDisposeModal(o) {
  ...
  updateDispWarn();
  $("matDispOverlay").classList.add("on");
}
function updateDispWarn() {
  const d = $("matDispDate").value;
  $("matDispWarn").style.display = (/^\d{4}-\d{2}-\d{2}$/.test(d) && d < todayISO()) ? "" : "none";
}
$("matDispDate").addEventListener("change", updateDispWarn);
$("matDispToday").onclick = () => { $("matDispDate").value = todayISO(); updateDispWarn(); };
$("matDispExpiry").onclick = () => { $("matDispDate").value = $("matDispExpiry").dataset.d || todayISO(); updateDispWarn(); };
$("matDispCancel").onclick = () => $("matDispOverlay").classList.remove("on");
$("matDispOk").onclick = async () => {
  const qty = Number(String($("matDispQty").value).replace(/,/g, "")) || 0;
  const date = $("matDispDate").value;
  const reason = $("matDispReason").value.trim() || "유통기한 만료";
  if (!(qty > 0)) return toast("폐기 수량을 입력하세요");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return toast("폐기일을 선택하세요");
  try {
    await api("/api/matdisposal", { method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ date, material_id: MDISP.mid, qty, reason, expiry: MDISP.expiry || "" }) });
    $("matDispOverlay").classList.remove("on");
    toast(`${MDISP.name} ${NF(qty)}${MDISP.unit || ""} · ${date} 폐기 처리되었습니다`);
    if (MDISP.onReload) MDISP.onReload();
  } catch (e) { /* api 토스트 */ }
};
async function openMatDispose(mid) {
  const m = materialById(mid) || {};
  const ex = (E.matExpiry || {})[mid];
  if (!ex || !(ex.expired > 0)) return;
  const exps = (ex.batches || []).map(b => b.exp).filter(Boolean).sort();
  openDisposeModal({ mid, name: m.name, unit: m.unit || "", expired: ex.expired,
    expiry: exps[0] || "", onReload: () => loadDay(E.date) });
}
async function undoMatDispose(did) {
  if (!confirm("이 폐기 기록을 취소할까요? — 재고가 복구됩니다")) return;
  try {
    await api("/api/matdisposal/" + did, { method: "DELETE" });
    toast("폐기가 취소되었습니다 — 재고가 복구되었습니다");
...
function renderMatStatus() {
  ...
  if (MATSTAT.filter === "disp") {
    $("msHead").innerHTML = ["폐기일", "자재명", "구분", "폐기수량", "유통기한", "사유", "처리"].map((h, i) =>
      `<th style="padding:6px 8px; border-bottom:2px solid var(--line); text-align:${i === 1 ? "left" : (i === 3 ? "right" : "center")}; font-size:12px; color:var(--muted);">${h}</th>`).join("");
    let dl = q ? disposals.filter(x => (x.name || "").toLowerCase().includes(q)) : disposals.slice();
    $("msBody").innerHTML = dl.length ? dl.map(x => {
      const u = esc(x.unit || "");
      return `<tr style="border-bottom:1px solid var(--line-soft);">
        <td style="padding:6px 8px; text-align:center; font-weight:700;">${esc(x.date)}</td>
        <td style="padding:6px 8px;"><button class="uselink" data-mshist="${x.material_id}" style="font-weight:600; text-align:left;">${esc(x.name)}</button></td>
        <td style="padding:6px 8px; text-align:center;"><span class="chip cat" style="font-size:10.5px;">${kindTxt(x.kind)}</span></td>
        <td style="padding:6px 8px; text-align:right; font-weight:700; color:#c0392b;">${NF(x.qty)}${u}</td>
        <td style="padding:6px 8px; text-align:center; color:var(--muted); font-size:11.5px;">${esc(x.expiry || "—")}</td>
        <td style="padding:6px 8px; text-align:center; font-size:11.5px;">${esc(x.reason || "")} ${x.auto ? '<span class="chip" style="background:#eef0f2; font-size:10px;">자동</span>' : '<span class="chip cat" style="font-size:10px;">수동</span>'}</td>
        <td style="padding:6px 8px; text-align:center;">${canStock ? `<button class="btn ghost sm" data-mdundo="${x.id}" style="padding:1px 8px;">취소</button>` : "—"}</td></tr>`;
    }).join("") : `<tr><td colspan="7" class="auto" style="padding:16px; text-align:center;">폐기 내역이 없습니다</td></tr>`;
    return;
  }
  // ── 재고 목록 — 전체 자재 현재 상태 표 ──
  if (MATSTAT.filter === "stock") {
    ...
    let list = q ? items.filter(x => (x.name || "").toLowerCase().includes(q)) : items.slice();
    $("msBody").innerHTML = list.length ? list.map(it => {
      const u = esc(it.unit || "");
      let exp = '<span class="auto">—</span>';
      if (it.expired > 0) exp = `<span style="color:#c0392b; font-weight:700;">⚠ 만료 ${NF(it.expired)}${u}</span>`;
      else if (it.exp) { const dd = it.exp_days; exp = (dd != null && dd <= (d.soon_days || 7)) ? `<span style="color:#B45309; font-weight:600;">⏰ ${esc(it.exp)}</span> <span class="auto" style="font-size:11px;">D-${dd}</span>` : `${esc(it.exp)}${dd != null ? ` <span class="auto" style="font-size:11px;">D-${dd}</span>` : ""}`; }
      let saf = it.safety > 0 ? `${NF(it.safety)}${u}` : '<span class="auto">—</span>';
      if (it.low) saf = `<span style="color:#B45309; font-weight:700;">${NF(it.safety)}${u}</span><div class="auto" style="font-size:11px; color:#B45309;">부족 ${NF(it.shortage)}${u}</div>`;
      return `<tr style="border-bottom:1px solid var(--line-soft);">
  ...
  items.filter(x => x.expired > 0).forEach(x => {
    ...
    events.push({ date: exps.length ? isoPlus(exps[0], 1) : "", type: "만료", material_id: x.id, name: x.name, unit: x.unit, kind: x.kind,
      content: `${NF(x.expired)}${x.unit || ""} 만료 · 미처리${exps.length ? ` (유통기한 ${exps.join(", ")})` : ""}`, color: "#c0392b",
      act: canStock ? `<button class="btn ghost sm" data-msdispose="${x.id}" style="color:#c0392b; border-color:#c0392b; padding:1px 8px;">확정 폐기</button>` : "" });
  });
  ...
  else if (MATSTAT.filter === "low") evs = events.filter(e => e.type === "부족");
  if (q) evs = evs.filter(e => (e.name || "").toLowerCase().includes(q));
  evs.sort((a, b) => (a.date < b.date ? 1 : (a.date > b.date ? -1 : 0)));   // 최신 발생일 위로
  const stChip = t => t === "만료" ? '<span class="chip" style="background:#fdecea; color:#c0392b; font-size:10px;">만료</span>'
    : '<span class="chip" style="background:#fdf0e2; color:#B45309; font-size:10px;">' + t + '</span>';
  $("msHead").innerHTML = ["발생일", "자재명", "구분", "상태", "내용", "처리"].map((h, i) =>
    `<th style="padding:6px 8px; border-bottom:2px solid var(--line); text-align:${i === 1 || i === 4 ? "left" : "center"}; font-size:12px; color:var(--muted);">${h}</th>`).join("");
  $("msBody").innerHTML = evs.length ? evs.map(e => `<tr style="border-bottom:1px solid var(--line-soft);">
    <td style="padding:6px 8px; text-align:center; font-weight:700;">${esc(e.date || "—")}</td>
    <td style="padding:6px 8px;"><button class="uselink" data-mshist="${e.material_id}" style="font-weight:600; text-align:left;">${esc(e.name)}</button></td>
...
function msDispose(mid) {
  ...
  const exps = (it.exp_batches || []).map(b => b.exp).filter(Boolean).sort();
  openDisposeModal({ mid, name: it.name, unit: it.unit || "", expired: it.expired,
    expiry: exps[0] || "", onReload: () => loadMatStatus() });
}
...
$("msBody").addEventListener("click", async e => {
  ...
  if (un) {
    ...
    if (!confirm(`${x ? x.name + " " : ""}폐기(${x ? x.date : ""})를 취소할까요?\n재고가 복구되고, 이 유통기한은 다시 자동 폐기되지 않습니다.`)) return;
    try {
      await api("/api/matdisposal/" + un.dataset.mdundo, { method: "DELETE" });
      toast("폐기가 취소되었습니다 — 재고 복구");
      loadMatStatus();
    } catch (e) { /* api 토스트 */ }
    return;
  }
...
function buildProdBody() {
  ...
  const body = {
    ...
    // 반제품 생산 (반제품=직접 만드는 자재) — 배합수 → 반제품 재고↑ + 원재료 자동 차감(서버가 semi_bom대로)
    semi_mat_prod: (E.semiProd || []).filter(r => r.material_id).map(r => ({
...
async function invQuery() {
  ...
  INV.data = d;
  $("invDoc").innerHTML = invoiceSheetHtml(d, $("invHideAmt").checked);
  $("invPrintBtn").style.display = (d.items || []).length ? "" : "none";
}
// 금액 숨김 토글 — 이미 조회된 명세서면 즉시 다시 그림
...
function invPrint() {
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>거래명세서 ${esc((INV.data.buyer || {}).name || "")}</title>
    ...
    ${invoiceSheetHtml(INV.data, $("invHideAmt").checked)}</body></html>`);
  w.document.close();
}
// 공급자(회사) 정보 편집
...
function loadPurchRep() {
  const t = todayISO();
  if (!$("prFrom").value) $("prFrom").value = t.slice(0, 4) + "-01-01";
  if (!$("prTo").value) $("prTo").value = t;
  if (!PREP.data) prQuery();
}
async function prQuery() {
  let d; try { d = await api(`/api/purchasereport?frm=${$("prFrom").value}&to=${$("prTo").value}`); } catch (e) { return; }
  PREP.data = d; renderPurchRep();
}
function renderPurchRep() {
  const d = PREP.data; if (!d) return;
  $("prSummary").innerHTML = `기간 ${esc(d.from)} ~ ${esc(d.to)} · 총 매입 <span style="color:var(--crit)">₩${NF(d.total.amount)}</span>`;
  const tab = PREP.tab, tot = d.total.amount || 1;
  if (tab === "partner" || tab === "material") {
    const arr = tab === "partner" ? d.by_partner : d.by_material;
    $("prHead").innerHTML = `<tr><th>${tab === "partner" ? "거래처" : "자재"}</th><th class="r">수량</th><th class="r">매입액(원)</th><th class="r">비중</th></tr>`;
    $("prBody").innerHTML = arr.map(r => `<tr>
      <td>${tab === "material" && r.code ? `<span class="num auto" style="font-size:10.5px;">${esc(r.code)}</span> ` : ""}<b>${esc(r.name)}</b></td>
      <td class="r">${NF(r.qty)}${tab === "material" && r.unit ? " " + esc(r.unit) : ""}</td><td class="r" style="font-weight:700;">${NF(r.amount)}</td>
      <td class="r auto">${(r.amount / tot * 100).toFixed(1)}%</td></tr>`).join("")
      || `<tr><td colspan="4" class="auto" style="padding:16px; text-align:center;">기간 내 매입(입고)이 없습니다</td></tr>`;
  } else {
    $("prHead").innerHTML = `<tr><th>월</th><th class="r">매입액(원)</th></tr>`;
    $("prBody").innerHTML = d.by_month.map(r => `<tr><td><b>${esc(r.month)}</b></td><td class="r" style="font-weight:700;">${NF(r.amount)}</td></tr>`).join("")
      || `<tr><td colspan="2" class="auto" style="padding:16px; text-align:center;">기간 내 매입이 없습니다</td></tr>`;
  }
}
$("prTabs").addEventListener("click", e => {
  const b = e.target.closest("[data-prtab]"); if (!b) return;
  PREP.tab = b.dataset.prtab;
  [...$("prTabs").children].forEach(x => x.classList.toggle("on", x === b));
  renderPurchRep();
});
...
function loadPnl() {
  const t = todayISO();
  if (!$("pnlFrom").value) $("pnlFrom").value = t.slice(0, 4) + "-01-01";
  if (!$("pnlTo").value) $("pnlTo").value = t;
  if (!PNL.data) pnlQuery();
}
async function pnlQuery() {
  let d; try { d = await api(`/api/pnl?frm=${$("pnlFrom").value}&to=${$("pnlTo").value}`); } catch (e) { return; }
  PNL.data = d; renderPnl();
}
function renderPnl() {
  const d = PNL.data; if (!d) return;
  const t = d.total;
  const card = (lbl, v, color) => v == null ? "" : `<div class="kpi" style="min-width:150px;"><div class="lbl">${lbl}</div><div class="val num" style="color:${color || ""}">₩${NF(v)}</div></div>`;
  $("pnlCards").innerHTML = card("매출", t.sales) + card("매입", t.buy, "var(--crit)") + card("노무비", t.labor, "var(--crit)")
    + card("간이 이익", t.profit, t.profit != null && t.profit < 0 ? "var(--crit)" : "var(--ok)")
    + (t.profit == null ? `<div class="kpi" style="min-width:150px;"><div class="lbl">간이 이익</div><div class="auto" style="font-size:12px; margin-top:6px;">매입·노무 권한 필요</div></div>` : "");
  const cols = ["<th>월</th>", "<th class='r'>매출</th>", d.see_buy ? "<th class='r'>매입</th>" : "", d.see_labor ? "<th class='r'>노무비</th>" : "", (d.see_buy && d.see_labor) ? "<th class='r'>이익</th>" : ""].join("");
  $("pnlHead").innerHTML = `<tr>${cols}</tr>`;
  $("pnlBody").innerHTML = d.rows.map(r => `<tr>
    <td><b>${esc(r.month)}</b></td>
    <td class="r">${NF(r.sales)}</td>
    ${d.see_buy ? `<td class="r">${NF(r.buy)}</td>` : ""}
    ${d.see_labor ? `<td class="r">${NF(r.labor)}</td>` : ""}
    ${(d.see_buy && d.see_labor) ? `<td class="r" style="font-weight:700; color:${r.profit < 0 ? "var(--crit)" : "var(--ok)"}">${NF(r.profit)}</td>` : ""}</tr>`).join("")
    || `<tr><td colspan="5" class="auto" style="padding:16px; text-align:center;">기간 내 데이터가 없습니다</td></tr>`;
}
...
async function openShipFromSO(target) {
  SFSO.target = target === "prod" ? "prod" : "ship";
  ...
  SFSO.orders = orders;
  _sfsoEnsureDom();
  const isProd = SFSO.target === "prod";
  $("sfsoTitle").textContent = isProd ? "📥 수주 불러오기 → 생산 계획" : "📥 수주 불러오기 → 완제품 출고";
  $("sfsoHint").textContent = isProd
    ? "고른 주문의 제품·수량을 [생산실적]의 '계획'으로 채웁니다."
    : "고른 주문의 제품·수량·거래처를 [완제품 출고] 행으로 채웁니다. 생산일자(LOT)는 채운 뒤 선택하세요.";
  $("sfsoDoneWrap").style.display = isProd ? "none" : "";
...
function _sfsoEnsureDom() {
  ...
  ov.innerHTML = `<div class="modal" style="width:min(560px,95vw);">
    <h3 style="margin:0 0 4px;" id="sfsoTitle">📥 수주 불러오기</h3>
    <p class="hint" id="sfsoHint"></p>
    <div id="sfsoBody" style="max-height:52vh; overflow:auto; border:1px solid var(--line); border-radius:8px; padding:4px 8px;"></div>
    <label id="sfsoDoneWrap" style="display:inline-flex; align-items:center; gap:6px; margin-top:10px; font-size:13px;">
...
async function _sfsoApply() {
  ...
  picked.forEach(oid => {
    ...
    (o.items || []).forEach(it => {
      if (it.product_id && it.qty > 0) {
        if (SFSO.target === "prod") E.prod.push({ product_id: it.product_id, line_id: null, batches: "", plan_qty: it.qty, prod_qty: "", defect_qty: "", lotSplits: [], expiry: "" });
        else E.ship.push({ product_id: it.product_id, partner_id: o.partner_id || null, qty: it.qty, prod_date: "", lotExpiry: "", lotNo: 0 });
        added++;
      }
    });
  });
  ...
  E.dirty = true;
  if (SFSO.target === "prod") renderProd(); else renderShip();
  const done = SFSO.target !== "prod" && $("sfsoDone").checked;
  ...
  toast(`수주 ${picked.length}건 · ${added}개 품목을 ${SFSO.target === "prod" ? "생산 계획" : "출고"}에 불러왔습니다${done ? " (주문 완료 처리)" : ""} — 확인 후 💾 저장하세요`);
}
{ const b = $("shipFromSO"); if (b) b.onclick = () => openShipFromSO("ship");
  const b2 = $("prodFromSO"); if (b2) b2.onclick = () => openShipFromSO("prod"); }
...
async function loadBom(pid) {
  ...
  BOM.rows = (await api("/api/bom/" + pid)).map(r => ({
    ...
    note: r.note || "" }));
  BOM.semis = [];   // (구) 반제품 재료 섹션 제거 — 반제품은 이제 자재로 배합비에 직접 넣는다
...
async function renderBomTab() {
  ...
  // 배합비 자재 검색 = 원·부재료 + 반제품(자재) (반제품도 빵 재료로 넣을 수 있음)
  $("qaMaterials").innerHTML = M.raw.concat(M.sub, M.semi || []).map(o => `<option value="${esc(o.name)}">`).join("");
  // 제품 검색 자동완성 전용 datalist — 완제품만 (반제품은 이제 자재)
  $("bomProdList").innerHTML = M.product.filter(p => p.status !== "단종")
...
function renderBomRows() {
  ...
  $("mBody").innerHTML = matSecHtml;   // 반제품은 이제 자재 — 위 자재 목록에서 원재료처럼 넣으면 됨 (별도 반제품 재료 섹션 제거)
}
...
const MFORMS = {
  ...
  semi: [["name", "반제품명 * (예: 발효종)"], ["code", "품목코드 (비우면 자동 부여 · 반제품 SF####)"], ["spec", "규격 (예: 발효반죽)"],
    ...
    ["stock_set", "현재고 (아래 기준일에 실사 기록)", "num"], ["stock_date", "기준일 (이 날짜에 기록)", "date"],
    ["status", "상태", "sel", ["사용중", "중단"]], ["note", "비고", "full"]],
  partner: [["name", "거래처명 *"], ["type", "유형 — 선택 또는 직접 입력 (예: 기부)", "combo", ["판매처", "자재 공급처", "용역업체"]],
    ["biz_no", "사업자등록번호"], ["ceo", "대표자명"],
    ["phone", "전화"], ["mobile", "모바일"],
    ["email", "이메일 (발주서 메일 발송)"],
    ["address", "주소 (거래명세서 공급받는자 주소)", "full"],
...
$("postBody").addEventListener("click", async e => {
  ...
  if (ur) {
    ...
    try {
      ...
      loadPoStat();
      loadLowStock();
      ENTRY_STALE = true;   // 일일 입력에 자동 반영되도록 — 다음 진입 시 재로드
    } catch (err) { /* api가 토스트 */ }
    return;
  }
...
const LEDGER = { data: null, showAll: false };
...
// 자주 쓰는 색 원클릭 (검정/빨강/파랑)
$("pmToolbar").addEventListener("click", e => {
  const c = e.target.closest("button[data-color]");
  if (!c) return;
  $("pmMsg").focus();
...
function openAnaP(pid) {
  ...
  if (_m) _m.style.width = "780px";   // 제품 상세는 기본 폭 (자재 이력이 넓혔던 것 되돌림)
...
async function openMatHistory(mid) {
  ...
  $("anaPBody").innerHTML = `<div style="display:flex; gap:16px; align-items:stretch; flex-wrap:wrap; max-height:76vh;">
    ...
    <tbody class="num">${d.rows.map(r => `<tr ${r.in_qty > 0 ? 'style="background:var(--ok-soft)"' : ""}>
      ...
      || '<tr><td colspan="8" class="auto">기록 없음</td></tr>'}</tbody></table></div></div></div>`;
...
const SCHED = { week: "", data: null, saved: "", weeks: [], kind: "ship", gridMode: (localStorage.getItem("ms_schedGrid") || "col"), editMode: false, dirty: false };
  ...
  return groups.map(g => {
    ...
    for (let k = 0; k < its.length; k++) {
      ...
      if (merged) {
        ...
        it.boxes = (q > 0 && pk > 0) ? String(Math.floor(q / pk + 1e-3)) : "";   // 합산 수량으로 박스 재계산
      }
    }
...
async function schedImportFromShip() {
  if (SCHED.kind !== "prod") return;
  if (SCHED.data && (SCHED.data.groups || []).length && !confirm("\ud604\uc7ac \uc0dd\uc0b0 \uc2a4\ucf00\uc904 \ud45c\ub97c \uc774 \uc8fc\uc758 \ucd9c\uace0 \uc2a4\ucf00\uc904 \ub0b4\uc6a9\uc73c\ub85c \ub36e\uc5b4\uc4f8\uae4c\uc694?")) return;
  try {
    const r = await api("/api/schedule?week=" + SCHED.week);
    const d = (r.data && Object.keys(r.data).length) ? r.data : null;
    if (!d) { toast("\uc774 \uc8fc\uc758 \ucd9c\uace0 \uc2a4\ucf00\uc904\uc774 \ube44\uc5b4 \uc788\uc2b5\ub2c8\ub2e4"); return; }
    SCHED.data = _schedNorm(JSON.parse(JSON.stringify(d)), SCHED.week);
    SCHED.dirty = true;
    renderSchedule();
    toast("\ucd9c\uace0 \uc2a4\ucf00\uc904\uc5d0\uc11c \ud45c\ub97c \ubd88\ub7ec\uc654\uc2b5\ub2c8\ub2e4 \u2014 \uc0dd\uc0b0\uc5d0 \ub9de\uac8c \ud3b8\uc9d1 \ud6c4 \uc800\uc7a5\ud558\uc138\uc694");
  } catch (e) { /* api \ud1a0\uc2a4\ud2b8 */ }
}
...
function _schedPickUndo(key) {
  ...
  (SCHED.data.groups || []).forEach(g => (g.items || []).forEach((x, i, arr) => {
    ...
    if (m) {
      ...
      if (x.srcMeta) hit.forEach(k => { delete x.srcMeta[k]; });
...
// 담긴 항목의 출처(카드) 메타 — 담을 때 저장하고(srcMeta), 생산 스케줄을 열 때마다 출고 스케줄의 최신 값으로 갱신된다.
// 예전 항목(속성 키만 있음)은 키 토막에서 읽는다.
function _schedSrcMeta(it, key) {
  const m = it && it.srcMeta && it.srcMeta[key];
  if (m && typeof m === "object") return m;
  if (!key || String(key).startsWith("u:")) return null;
  const seg = _schedKeySegs(key);
  return { attr: key, label: seg[0] || "", partner: seg[1] || "", shipDate: seg[2] || "", pack: seg[3] || "", expiry: seg[4] || "", expiry2: seg[5] || "", perBox: seg.length >= 7 ? seg[6] : "" };
}
function _schedPerBoxOfSrc(it, key) { const m = _schedSrcMeta(it, key); const n = m ? Number(m.perBox) : 0; return isFinite(n) && n > 0 ? n : 0; }
// 항목의 박스 수 — 출처에 '박스당 개수'가 있으면 그걸로, 없으면 개입으로
function _schedBoxesFor(it, qty, pack) {
  const q = _schedQtyNum(qty), pb = _schedPerBoxOfSrc(it, it && it.src), pk = _schedQtyNum(pack);
  if (q > 0 && pb > 0) return String(Math.floor(q / pb + 1e-3));
  return (q > 0 && pk > 0) ? String(Math.floor(q / pk + 1e-3)) : "";
}
// 출고 표의 줄 하나 = 카드 하나. 키 = "u:<uid>"(출고 항목 고유번호, 서버가 부여) — uid가 아직 없는 항목은 속성 키(제품|거래처|출고일|개입|소비|예정|박스당개수).
// 반환 keyOf(Map item→key). 카드 메타는 cardMeta(key→{attr, label, partner, shipDate, pack, expiry, expiry2, perBox, qty, week})에 채움.
function _schedShipKeys(groups, week, cardMeta) {
  const cnt = new Map(), keyOf = new Map();
  groups.forEach(g => (g.items || []).forEach(it => {
    if (!it || !(it.label || "").trim()) return;
    const attr0 = [(it.label || "").trim(), (it.partner || g.partner || "").trim(), g.shipDate || "", String(it.pack || "").trim(), (it.expiry || "").trim(), (it.expiry2 || "").trim(), _schedPerBoxOf(it)].join("|");
    const n = cnt.get(attr0) || 0; cnt.set(attr0, n + 1);
    const attr = n ? attr0 + "#" + (n + 1) : attr0;
    const key = it.uid ? "u:" + it.uid : attr;
    keyOf.set(it, key);
    if (cardMeta) cardMeta[key] = { attr, label: (it.label || "").trim(), partner: (it.partner || g.partner || "").trim(), shipDate: g.shipDate || "", pack: String(it.pack || "").trim(), expiry: (it.expiry || "").trim(), expiry2: (it.expiry2 || "").trim(), perBox: _schedPerBoxOf(it), qty: _schedQtyNum(it.qty), week: week || "" };
  }));
  return keyOf;
}
...
function _schedCardAttr(k) { const cm = (typeof SPICK !== "undefined" && SPICK.cardMeta) || {}; return (cm[k] && cm[k].attr) || k; }
function _schedKeyResolve(a) {
  ...
  if (String(a).startsWith("u:")) return null;   // uid 키인데 카드가 없음 = 출고 항목이 지워짐 → 연결 안 됨
  const sa = _schedKeySegs(a); if (sa.length < 3) return null;
  const same = (k, n) => { const sk = _schedKeySegs(_schedCardAttr(k)); if (sk.length < n) return false; for (let i = 0; i < n; i++) if (sk[i] !== sa[i]) return false; return true; };
...
function _schedKeyEq(a, b) {
  if (a === b) return true;
  if (String(a).startsWith("u:")) return false;                              // uid 키는 정확히 같을 때만
  if (String(b).startsWith("u:")) return _schedKeyResolve(a) === b;          // 예전 속성 키 → 카드(uid)로 해석
...
// 담긴 항목을 현재 카드 목록(SPICK.keys/cardMeta, 출고 주 scopeWeek)에 맞춰 갱신:
//  1) 예전 속성 키는 맞는 카드의 uid 키로 바꾼다(출고 스케줄을 고쳐도 연결 유지). 다른 주 출고분 키는 건드리지 않는다.
//  2) 연결된 출처의 메타(소비기한·개입·박스당개수)를 출고 스케줄 최신 값으로 갱신하고, 출처가 하나뿐인 칸은 칸의 소비기한·개입·박스도 따라 바꾼다.
function _schedRelinkPlaced(scopeWeek) {
  if (!SCHED.data || !Array.isArray(SPICK.keys)) return 0;
  const cm = SPICK.cardMeta || {}; let n = 0;
  const inScope = () => true;   // 속성 키는 카드 목록에서 해석되면 연결(출고 스케줄엔 다른 주 날짜도 들어가므로 주로 거르지 않음)
  (SCHED.data.groups || []).forEach(g => (g.items || []).forEach(x => {
    if (!x || x.spacer || !(x.label || "").trim()) return;
    const cur = _schedItemSrcs(x); if (!cur) return;
    const out = {}; let ch = false;
    Object.keys(cur).forEach(k => {
      let t = k;
      if (!SPICK.keys.includes(k) && inScope(k)) { const r = _schedKeyResolve(k); if (r) t = r; }
      if (t !== k) ch = true;
      out[t] = _schedQtyNum(out[t] || 0) + _schedQtyNum(cur[k]);
    });
    const meta = Object.assign({}, x.srcMeta || {});
    Object.keys(out).forEach(k => { if (cm[k]) { const nm = Object.assign({}, cm[k]); if (JSON.stringify(meta[k] || null) !== JSON.stringify(nm)) { meta[k] = nm; ch = true; } } });
    Object.keys(meta).forEach(k => { if (!(k in out)) delete meta[k]; });
    if (!ch) return;
    n++;
    const keys = Object.keys(out);
    if (keys.length === 1) { x.src = keys[0]; delete x.srcs; } else { x.srcs = out; x.src = keys[0]; }
    x.srcMeta = meta;
    if (keys.length === 1 && meta[keys[0]]) {   // 출처가 하나 → 칸의 소비기한·개입도 출고 최신 값으로
      const mm = meta[keys[0]];
      x.expiry = mm.expiry || ""; x.expiry2 = mm.expiry2 || ""; if (mm.pack) x.pack = mm.pack;
    }
    if (x.boxesAuto !== false) x.boxes = _schedBoxesFor(x, x.qty, x.pack);
  }));
...
// 생산 스케줄을 열 때: 담긴 항목들이 참조하는 출고 주(이번 주 + 메타의 주)를 읽어 소비기한·개입·박스를 최신으로 맞춘다
async function _schedRefreshFromShip() {
  if (SCHED.kind !== "prod" || !SCHED.data) return;
  const weeks = new Set([SCHED.week]);
  (SCHED.data.groups || []).forEach(g => (g.items || []).forEach(x => {
    const cur = x && _schedItemSrcs(x); if (!cur) return;
    Object.keys(cur).forEach(k => { const m = _schedSrcMeta(x, k); if (m && m.week) weeks.add(m.week); else if (m && m.shipDate) weeks.add(_schedMonday(m.shipDate)); });
  }));
  const list = [...weeks].filter(Boolean).slice(0, 6);
  const keep = { keys: SPICK.keys, cardMeta: SPICK.cardMeta };
  let changed = 0;
  for (const w of list) {
    let d = null; try { const r = await api("/api/schedule?week=" + w); d = r && r.data; } catch (e) { d = null; }
    if (!d || !Array.isArray(d.groups)) continue;
    const cm = {}; const keyOf = _schedShipKeys(d.groups, w, cm);
    SPICK.keys = [...keyOf.values()]; SPICK.cardMeta = cm;
    changed += _schedRelinkPlaced(w);
  }
  SPICK.keys = keep.keys; SPICK.cardMeta = keep.cardMeta;
  return changed;
}
...
function _schedMergeInto(dest, addQty, addSrcs, addMeta) {
  ...
  if (addMeta) dest.srcMeta = Object.assign({}, dest.srcMeta || {}, addMeta);
...
function _schedPickRender() {
  ...
  const cm = {}; const keyOf = _schedShipKeys(groups, SPICK.week || SCHED.week, cm);   // 출고 표의 줄 하나 = 카드 하나(uid 키)
  SPICK.keys = [...keyOf.values()]; SPICK.cardMeta = cm;
  if (_schedRelinkPlaced(SPICK.week || SCHED.week)) renderSchedDoc();   // 출고 스케줄이 바뀐 뒤에도 담긴 항목이 카드에 계속 연결·최신 표시
  ...
  const cols = groups.map(g => {
    const cards = (g.items || []).filter(it => (it.label || "").trim()).map(it => {
      ...
      const payload = esc(JSON.stringify({ label: it.label, qty: partial ? String(rem) : it.qty, shipQty: shipQ, src: key, meta: cm[key] || null, pack: it.pack, partner: it.partner || g.partner || "", expiry: it.expiry, expiry2: it.expiry2 }));
  ...
  if ($("schedPickCount")) $("schedPickCount").textContent = "제품을 아래 생산 표의 요일 열로 끌어다 놓으세요";
}
...
function _schedPickAdd(it, gcol, grow) {
  ...
  if (k == null || k < 0) { k = g.items.findIndex((x, i) => isEmpty(i)); if (k < 0) k = g.items.length; }
    ...
    if ((occ.label || "").trim() === (it.label || "").trim()) {   // 같은 제품 위에 놓음 → 수량 합치기
      _schedMergeInto(occ, qty, it.src ? { [it.src]: _schedQtyNum(qty) } : null, (it.src && it.meta) ? { [it.src]: it.meta } : null);
  ...
  ni.srcMeta = (it.src && it.meta) ? { [it.src]: it.meta } : undefined;
...
function _schedMoveItem(fromGc, fromGr, toGc, toGr) {
  ...
  if (destFilled && (dest.label || "").trim() === (from.label || "").trim()) {   // 같은 제품 → 수량 합치기
    _schedMergeInto(dest, from.qty, _schedItemSrcs(from), from.srcMeta || null);
...
// 마우스로 창 크기 조절 — 우하단 핸들을 끌면 비율(zoom)만 바뀜
let _pickResize = null;
function _schedPickInitResize() {
  const h = $("schedPickResize"), modal = $("schedPickModal");
  if (!h || !modal || h._wired) return; h._wired = true;
  h.addEventListener("mousedown", e => {
    const z = _schedPickZoom() || 1, r = modal.getBoundingClientRect();
    _pickResize = { x: e.clientX, w: r.width, baseW: (r.width / z) || 1 };
    e.preventDefault(); e.stopPropagation();
  });
  document.addEventListener("mousemove", e => {
    if (!_pickResize) return;
    let z = (_pickResize.w + (e.clientX - _pickResize.x)) / _pickResize.baseW;
    z = Math.max(0.5, Math.min(1.6, Math.round(z * 100) / 100));
    try { localStorage.setItem("schedPickZoom", z); } catch (x) {}
    _schedPickApplyZoom();
  });
...
if ($("schedPickZoomIn")) $("schedPickZoomIn").onclick = () => _schedPickSetZoom(0.1);
...
function _schedMonday(iso) {
  ...
  if (isNaN(d)) return _schedMonday(todayISO());
...
function _schedUid() { const a = new Uint8Array(4); (window.crypto || {}).getRandomValues ? crypto.getRandomValues(a) : a.forEach((_, i) => { a[i] = Math.floor(Math.random() * 256); }); return [...a].map(b => b.toString(16).padStart(2, "0")).join(""); }
function _schedBlankItem() { const o = { product_id: null, label: "", qty: "", pack: "", boxes: "", boxesAuto: true, expiry: "", expiry2: "", partner: "", memo: "", color: "" }; if (typeof SCHED !== "undefined" && SCHED.kind === "ship") o.uid = _schedUid(); return o; }
...
function _schedNorm(d, week) {
  ...
  if (!d.elem || typeof d.elem !== "object") d.elem = {};
  for (const k of ["label", "qty", "sub", "date"]) { d.elem[k] = d.elem[k] || {}; if (!(d.elem[k].s > 0)) d.elem[k].s = 100; d.elem[k].c = d.elem[k].c || ""; }
...
async function loadSchedule(dateOpt) {
  const week = _schedMonday(dateOpt || SCHED.week || todayISO());
  try {
    const r = await api(_schedBase() + "?week=" + week);
  ...
  if (SCHED.kind === "prod") {   // 출고 스케줄에서 고친 소비기한·개입·박스를 담긴 항목에 반영 (바뀐 게 있으면 저장 필요 표시)
    let ch = 0; try { ch = await _schedRefreshFromShip(); } catch (e) { ch = 0; }
    if (ch) { SCHED.dirty = true; SCHED.saved += " · 출고 스케줄 변경 반영됨(저장 필요)"; }
  }
...
// (붙여넣기 버튼은 각 제품군 머리의 [복사] 옆에 표시된다 — gctl + data-schedgpaste 핸들러 참고)
  ...
  const cards = d.groups.map((g, gi) => {
    ...
    return `<div class="card" style="padding:10px 12px; margin-bottom:10px; border:1px solid var(--line);">
      <div style="display:flex; gap:8px; align-items:center; margin-bottom:8px; flex-wrap:wrap;">
        ...
        <input data-g="${gi}" data-f="shipDate" class="datepick" readonly value="${esc(g.shipDate)}" placeholder="📅 출고일" style="${IN} width:130px;">
        <input data-g="${gi}" data-f="memo" value="${esc(g.memo)}" placeholder="제품군 비고" style="${IN} flex:1 1 140px;">
...
function _schedSetField(gi, ii, f, val) {
  ...
  if ((f === "qty" || f === "pack") && it.boxesAuto !== false) {
    ...
    it.boxes = (q > 0 && p > 0) ? (_schedPerBoxOfSrc(it, it.src) > 0 ? String(Math.floor(q / _schedPerBoxOfSrc(it, it.src) + 1e-3)) : String(Math.floor(q / p + 1e-3))) : "";
  }
}
...
function _schedFieldUpdate(e) {
  ...
  if (!hasG) { d[f] = inp.value; return; }
  const g = d.groups[+inp.dataset.g]; if (!g) return;
  const hasI = inp.dataset.i != null && inp.dataset.i !== "";
  if (hasI) {
    const it = g.items[+inp.dataset.i]; if (!it) return;
    it[f] = (f === "qty" || f === "pack") ? inp.value.replace(/,/g, "") : inp.value;
    if (f === "label") { const p = (M.product || []).find(x => x.name === inp.value); it.product_id = p ? p.id : null; }
...
function _schedClick(e) {
  ...
  if (gcp) {   // 제품군 복사 — 오른쪽에 바로 복제 + 다른 주에도 붙여넣을 수 있게 클립보드에 저장
    ...
    if (src) {
      ...
      _schedClipSet(src);   // 주 이동 후 각 제품군의 '붙여넣기'로 붙일 수 있도록 보관
      renderSchedDoc();
      toast(`'${src.name || "제품군"}'을(를) 복사했습니다 — 다른 주로 이동해도 [복사] 옆 [📋붙여넣기]로 붙일 수 있습니다`);
    }
    return;
  }
  const gpst = e.target.closest("[data-schedgpaste]");
  if (gpst) {   // 복사해 둔 제품군을 이 열 오른쪽에 붙여넣기 (출고일·소비기한 날짜는 이번 주로 자동 이동)
    const gi = +gpst.dataset.schedgpaste, clip = _schedClipGet();
    if (!clip) { toast("붙여넣을 제품군이 없습니다 — 먼저 제품군의 [복사]를 누르세요"); return; }
    const g = JSON.parse(JSON.stringify(clip.group));
    const wrap = { shipDates: [], groups: [g] };   // 주간 shipDates는 건드리지 않도록 임시 래퍼
    _schedShiftDates(wrap, _schedDayDiff(clip.week || SCHED.week, SCHED.week));
    SCHED.data.groups.splice(gi + 1, 0, g);
    _schedNorm(SCHED.data, SCHED.week);
    renderSchedDoc();
    toast(`'${g.name || "제품군"}'을(를) 붙여넣었습니다. 필요한 부분 수정 후 💾 저장하세요`);
    return;
  }
  ...
  if (ib) {   // 이 항목 위에 '빈 칸(공백)' 삽입 → 한 칸 아래로 띄움. 항목이 아니라 공백(spacer)이다.
    ...
    const sp = _schedBlankItem(); sp.spacer = true;
    SCHED.data.groups[gi].items.splice(ii, 0, sp);
    renderSchedDoc(); return;
  }
  const rh = e.target.closest("[data-schedrowh]");
  if (rh) {   // 이 행(줄) 높이 조절 (가중치 ±)
    const [k, dir] = rh.dataset.schedrowh.split(":").map(Number);
    if (!Array.isArray(SCHED.data.rowW)) SCHED.data.rowW = [];
    const cur = SCHED.data.rowW[k] > 0 ? SCHED.data.rowW[k] : 1;
    SCHED.data.rowW[k] = Math.max(0.4, Math.min(4, cur + dir * 0.2));
    renderSchedDoc(); return;
  }
  const gw = e.target.closest("[data-schedgwidth]");
  if (gw) {   // 이 제품군 열 폭 조절 (가중치 ±) — 좁게/넓게
    const [gi, dir] = gw.dataset.schedgwidth.split(":").map(Number);
    const g = SCHED.data.groups[gi]; if (!g) return;
    g.w = Math.max(0.4, Math.min(4, (g.w > 0 ? g.w : 1) + dir * 0.2));
    renderSchedDoc(); return;
  }
  const gm = e.target.closest("[data-schedgmove]");
  if (gm) {
    const [gi, dir] = gm.dataset.schedgmove.split(":").map(Number);
    const a = SCHED.data.groups, j = gi + dir;
    if (j >= 0 && j < a.length) { const t = a[gi]; a[gi] = a[j]; a[j] = t; renderSchedDoc(); }
    return;
  }
  ...
  if (mv) {
    ...
    if (j >= 0 && j < arr.length) { const t = arr[ii]; arr[ii] = arr[j]; arr[j] = t; renderSchedDoc(); }
    return;
  }
...
function _schedBoxText(q, per) {
  q = Number(q) || 0; per = Number(per) || 0; if (!(q > 0 && per > 0)) return "";
  const n = Math.floor(q / per + 1e-3), rem = Math.max(0, Math.round(q - n * per));
  return n.toLocaleString("ko-KR") + "박스" + (rem > 0 ? "+" + rem.toLocaleString("ko-KR") : "");
}
function _schedPackBox(it) {
  ...
  const q0 = Number(String(it.qty).replace(/,/g, "")), p0 = Number(packN);
  if (!boxes || it.boxesAuto !== false) {   // 박스 미입력(또는 자동 계산) 시 개입수로 환산 — 남는 낱개는 "+N" (직접 입력한 박스만 그대로)
    const per = (_schedPerBoxOfSrc && _schedPerBoxOfSrc(it, it.src)) || p0;
    if (!isNaN(q0) && per > 0 && q0) boxes = _schedBoxText(q0, per);
  } else if (/^[0-9,]+$/.test(boxes)) {
    boxes = Number(boxes.replace(/,/g, "")).toLocaleString("ko-KR") + "박스";
  }
  return pack && boxes ? `${pack}/${boxes}` : (pack || boxes || "");
}
// 제품군 열 폭 — 각 제품군의 가중치 g.w(기본 1)에 비례한 % 폭. 사용자가 열마다 좁게/넓게 조절.
function _schedGroupCols(groups) {
  const T = groups.reduce((s, g) => s + (g.w > 0 ? g.w : 1), 0) || 1;
  return groups.map(g => `<col style="width:${(((g.w > 0 ? g.w : 1) / T) * 100).toFixed(3)}%;">`).join("");
}
function buildScheduleDoc(d, week) {
  ...
  // 요소별(제품명·수량·개입박스·날짜) 크기(%)·색 — 표시 설정에서 조절
  const EL = d.elem || {};
  ...
  const expRow = groups.map(g => {
    ...
    return `<td style="${TD} text-align:center; font-weight:700; font-size:${dateSize}px; color:${ec("date", "inherit")};">${exps.length ? exps.map(esc).join("<br>") : "—"}</td>`;
  }).join("");
  const exp2Row = groups.map(g => {
    const exps = [...new Set((g.items || []).map(it => it.expiry2).filter(Boolean))];
    return `<td style="${TD} text-align:center; font-size:${dateSize}px; color:${ec("date", "#666")};">${exps.length ? exps.map(esc).join("<br>") : "—"}</td>`;
  }).join("");
...
// 편집형 미리보기 — buildScheduleDoc와 '같은 레이아웃'이되 셀 값이 입력칸(sched-ei)이다. 편집=미리보기 한 화면.
// data-* 속성은 카드 편집과 동일 → _schedFieldUpdate/_schedClick/_schedPaste를 그대로 재사용한다. 인쇄는 데이터에서 다시 그리므로 영향 없음.
function buildScheduleDocEdit(d, week) {
  const groups = d.groups || [];
  const n = groups.length || 1;
  const scale = d.fontScale > 0 ? d.fontScale : 1;
  const S = px => Math.max(8, Math.round(px * scale));
  const fam = d.fontFamily || "inherit";
  const ink = d.textColor || "#111";
  const baseFs = n <= 3 ? 20 : n <= 4 ? 18 : n <= 5 ? 17 : n <= 6 ? 16 : n <= 7 ? 15 : 14;
  const fs = S(baseFs), nameFs = S(baseFs + 3), titleFs = S(30), labelFs = S(baseFs + 1), qtyFs = S(baseFs + 8);
  ...
  const itemCellEdit = (g, gi, ii, pad, span) => {
    ...
    return `<td style="${TDI} ${P}${_prod ? " vertical-align:middle;" : ""}" data-gcol="${gi}" data-grow="${ii}"${RS}><div class="sched-celledit${(it.label || "").trim() ? " sched-cell-grab" : ""}" style="position:relative;${_prod ? " text-align:center;" : ""}">
      ...
      <div style="display:flex; align-items:center; gap:1px; font-size:${subSize}px; flex-wrap:wrap;">
        ...
        ${it_(gi, ii, "boxes", it.boxes, "", ` inputmode="numeric" placeholder="자동" style="width:40px; text-align:right; color:${ec("sub", "#c26a1f")}; font-weight:700;"`)}<span style="color:${ec("sub", "#c26a1f")};">박스</span>${(() => { if (it.boxesAuto === false) return ""; const q = _schedQtyNum(it.qty), per = _schedPerBoxOfSrc(it, it.src) || _schedQtyNum(it.pack); if (!(q > 0 && per > 0)) return ""; const rem = Math.round(q - Math.floor(q / per + 1e-3) * per); return rem > 0 ? `<span style="color:${ec("sub", "#c26a1f")}; font-weight:700;" title="박스로 나누고 남는 낱개">+${rem}</span>` : ""; })()}</div>
...
async function saveSchedule() {
  ...
  try {
    await api(_schedBase(), { method: "POST", headers: { "Content-Type": "application/json" },
...
function schedExportCsv() {
  const d = SCHED.data; if (!d) return;
  const lines = [_SCHED_CSV_HEAD.join(",")];
  (d.groups || []).forEach(g => {
  ...
  a.download = `주간스케줄_${SCHED.week || "sched"}.csv`;
  document.body.appendChild(a); a.click();
  setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 500);
  toast("엑셀(CSV)로 내보냈습니다");
}
// 단순 CSV 파서(따옴표·쉼표·개행). 쉼표 없고 탭만 있으면 TSV로 인식.
...
function schedImportCsv(text) {
  if (!SCHED.data) return;
  let rows = _parseDelimited(text);
  if (!rows.length) { toast("불러올 내용이 없습니다"); return; }
  const h0 = (rows[0] || []).map(s => String(s).trim());
  if (h0.includes("제품군") || h0.includes("제품명") || h0.includes("품목")) rows = rows.slice(1);
  if (!rows.length) { toast("데이터 행이 없습니다 (헤더만 있음)"); return; }
  const groups = [], idx = {};
  rows.forEach(r => {
  ...
  if (!confirm(`불러온 데이터로 이번 주 스케줄을 덮어쓸까요?\n제품군 ${groups.length}개 · 항목 ${totalItems}개\n(저장 전이라 되돌릴 수 없습니다)`)) return;
...
function schedFullscreen() {
  const w = window.open(location.pathname + "#schedfull=" + encodeURIComponent(SCHED.week), "_blank");
...
async function schedEnterFullMode(week) {
  ...
  const load = async () => {
    let d;
    try { const r = await api(_schedBase() + "?week=" + week); d = _schedNorm((r.data && Object.keys(r.data).length) ? r.data : _schedBlank(week), week); }
...
(function wireSchedule() {
  ...
  on("schedColEqual", () => {
    if (!SCHED.data) return;
    SCHED.data.groups.forEach(g => { g.w = 1; });
    renderSchedDoc(); toast("모든 열 폭을 똑같이 맞췄습니다");
  });
  on("schedRowEqual", () => {
    if (!SCHED.data) return;
  ...
  if (impf) impf.addEventListener("change", e => {
    ...
    const rd = new FileReader(); rd.onload = () => { schedImportCsv(String(rd.result || "")); e.target.value = ""; }; rd.readAsText(file, "utf-8");
  });
  // 편집=미리보기: 모든 편집 이벤트를 미리보기(#schedDoc)에서 받는다 (편집 모드에서만 입력칸이 있음)
  const del = $("schedDoc");
  if (del) { del.addEventListener("input", _schedFieldUpdate); del.addEventListener("change", _schedFieldUpdate); del.addEventListener("click", _schedClick); del.addEventListener("paste", _schedPaste); }
})();
window.addEventListener("load", () => {
  const m = (location.hash || "").match(/#schedfull=([^&]+)/);
  if (m) setTimeout(() => schedEnterFullMode(decodeURIComponent(m[1])), 700);
});
