...
MASTER_TABLES = {
    ...
    "staff": ("staff", ["name", "kind", "position", "process", "wage", "join_date", "leave_date", "phone", "status", "note"]),
...
def mat_history(mid: int, limit: int = 40):
    ...
    try:
        ...
        # 유통기한 만료 확정 폐기 — 날짜별 폐기량·사유·기한 (이력에 '폐기 행'으로 표시)
        disp = {}
        for r in con.execute("""SELECT date, SUM(qty) q, GROUP_CONCAT(DISTINCT expiry) exps,
...
# ── 공급자(회사) 정보 — 거래명세서·발주서 상단에 쓰는 우리 회사 정보 ──
CO_KEYS = ["co_name", "co_bizno", "co_ceo", "co_addr", "co_tel", "co_type", "co_item"]


@app.get("/api/company")
def company_get():
    d = {k: get_app_setting(k, "") for k in CO_KEYS}
    if not d["co_name"]:
        d["co_name"] = "리바이프로덕트 (REBYPRODUCT)"
    return d


@app.post("/api/company")
def company_set(request: Request, body: dict):
    require_admin(request)
    for k in CO_KEYS:
        if k in body:
            set_app_setting(k, str(body.get(k) or ""))
    return {"ok": True}


@app.get("/api/invoice")
def invoice(request: Request, partner_id: int = 0, frm: str = "", to: str = "", taxfree: int = 0):
    """거래명세서 — 거래처·기간별 출고 내역 + 공급자/공급받는자 + 공급가액·세액·합계."""
    if not (mcan(request, "prod_price") or mcan(request, "ship_amt")):
        raise HTTPException(403, "금액(단가) 열람 권한이 없습니다")
    con = connect()
    try:
        today = dt.date.today()
        frm = frm or today.replace(day=1).isoformat()
        to = to or today.isoformat()
        supplier = company_get()
        buyer = None
        if partner_id:
        ...
        supply = 0.0
        for it in items:
            price = it["uprice"] if it["uprice"] > 0 else (pp.get(it["pid"]) or it["base_price"] or 0)
...
def calendar_dates(ym: str, src: str = ""):
    ...
    try:
        if src == "staffing":
            ...
            # 자재 관련 '데이터가 있는 날' — 유통기한 도래일(만료) + 자재 입고·실사·사용·폐기 기록일
            exp = set()
            for b in con.execute("""SELECT mi.date, mi.made_date, mi.expiry, COALESCE(m.shelf_days,0) sd
                    FROM material_in mi JOIN material m ON m.id=mi.material_id"""):
                e = (b["expiry"] or "").strip()
                if not e and b["sd"] and int(b["sd"]) > 0:
                    base = (b["made_date"] or b["date"] or "").strip()
                    try:
                        e = (dt.date.fromisoformat(base) + dt.timedelta(days=int(b["sd"]))).isoformat()
                    except (ValueError, TypeError):
                        e = ""
                if e and e[:7] == ym:
                    exp.add(e)
            act = {r["date"] for r in con.execute("""
                SELECT date FROM material_in    WHERE substr(date,1,7)=?
                UNION SELECT date FROM material_usage WHERE substr(date,1,7)=?
                UNION SELECT date FROM material_daily WHERE substr(date,1,7)=? AND src='manual'
                UNION SELECT date FROM material_disposal WHERE substr(date,1,7)=?""", (ym, ym, ym, ym))}
            ds = sorted(exp | act)
        else:
...
@app.post("/api/verify-pw")
def verify_pw(request: Request, body: dict):
    """현재 로그인 사용자의 비밀번호 확인 — 현장 전체화면 잠금 해제용."""
    u = request.state.user
    con = connect()
    try:
        row = con.execute("SELECT pw_hash FROM users WHERE id=?", (u["id"],)).fetchone()
