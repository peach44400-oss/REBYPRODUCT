...
REM exe는 영문 이름(RebyStock.exe)으로 업로드 — 한글 파일명은 릴리스에서 default.exe로 깨짐
copy /y "dist\재고관리.exe" "dist\RebyStock.exe" >nul
gh release create "v%VER%" "dist\RebyStock.exe" "version.json" --title "v%VER%" --notes-file version.json
if errorlevel 1 (
